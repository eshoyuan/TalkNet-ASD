...
def evaluate_network(files, args):
    ...
    durationSet = {6} # Use this line can get more reliable result
    loadTime = 0
    forwardTime = 0
    for file in tqdm.tqdm(files, total = len(files)):
        ...
        forwardTime += time.time() - startForward
        allScore = numpy.round((numpy.mean(numpy.array(allScore), axis = 0)), 1).astype(float)
        allScores.append(allScore)
    sys.stderr.write("Load time: %.3f, Forward time: %.3f \r\n"%(loadTime, forwardTime))
    return allScores
